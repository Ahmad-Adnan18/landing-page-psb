...
const scrollToHash = (hash) => {
    if (!hash || typeof window === 'undefined' || !hash.startsWith('#')) return;
...
const navigateTo = (hash, closeMenu = false) => {
    scrollToHash(hash);
    if (closeMenu) {
...
<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-kun-primary via-[#A9262D] to-[#8B1E24] font-sans text-white overflow-hidden selection:bg-kun-gold selection:text-kun-primary">
        ...
        <nav 
            ...
            :class="{ 
                'bg-kun-primary/100 backdrop-blur-md shadow-lg py-4': isScrolled || isMenuOpen,
            ...
            <div class="container mx-auto px-4">
                <div class="flex justify-between items-center transition-all duration-300">
                    ...
                    <div class="flex items-center gap-3">
                        <img src="/images/logo.png" alt="Logo" class="h-12 w-auto rounded-lg shadow-lg bg-white p-1" />
                    </div>
                ...
                <div v-if="isMenuOpen" class="md:hidden py-4 border-t border-white/10">
                    <div class="flex flex-col gap-4">
                        <a href="#home" @click.prevent="navigateTo('#home', true)" class="text-sm font-medium text-white">Home</a>
                        <a href="#features" @click.prevent="navigateTo('#features', true)" class="text-sm font-medium text-white">Fitur</a>
                        <a href="#solusi" @click.prevent="navigateTo('#solusi', true)" class="text-sm font-medium text-white">Solusi</a>
                        <a href="#pricing" @click.prevent="navigateTo('#pricing', true)" class="text-sm font-medium text-white">Harga</a>
                        <a href="#testimoni" @click.prevent="navigateTo('#testimoni', true)" class="text-sm font-medium text-white">Testimoni</a>
        ...
        <!-- Hero Section -->
        <section id="home" class="relative min-h-screen lg:h-screen pt-28 pb-16 lg:pt-28 lg:pb-10 overflow-visible lg:overflow-hidden scroll-mt-28 md:scroll-mt-32">
            <div class="container mx-auto px-4 relative z-6">
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div class="text-center lg:text-left">
                        <h1 
                            class="text-6xl md:text-6xl lg:text-6xl font-extrabold text-white leading-tight mb-8 min-h-[160px] md:min-h-[160px] lg:min-h-[140px]"
                            data-aos="fade-right"
                        >
                            <TextType 
                        ...
                        >
                            ...
                            <a 
                                href="#features"
                                @click.prevent="navigateTo('#features')" 
        ...
        <!-- Features Section -->
        <section id="features" class="py-20 bg-white text-slate-800 scroll-mt-28 md:scroll-mt-32">
            <div class="container mx-auto px-4">
        ...
        <!-- Gallery Section -->
        <section class="py-10 bg-gradient-to-br from-kun-primary to-[#D93030] overflow-hidden mt-10 scroll-mt-28 md:scroll-mt-32">
                ...
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div data-aos="fade-left">
                        ...
                        <ul class="space-y-4">
                            ...
                            <li class="flex items-center gap-3">
                                <CheckCircle2 class="w-5 h-5 text-kun-primary" />
                                <span>Cetak kartu hasil ujian otomatis</span>
                            </li>
                            <li class="flex items-center gap-3">
                                <CheckCircle2 class="w-5 h-5 text-kun-primary" />
        ...
        <section id="testimoni" class="py-20 bg-slate-50 text-slate-800">
            <div class="container mx-auto px-4">
                <div class="text-center mb-16">
                    <span class="text-kun-primary font-semibold tracking-[0.3em] uppercase text-xs mb-3 block" data-aos="fade-up">Testimoni</span>
                    <h3 class="text-3xl font-bold text-slate-900 mb-4" data-aos="fade-up">Apa Kata Mereka?</h3>
                    <p class="text-slate-600" data-aos="fade-up" data-aos-delay="100">Cerita sukses dari pengelola pesantren yang sudah bertransformasi digital</p>
                </div>

                <div class="relative">
                    <div class="absolute -inset-x-10 -top-6 h-48 bg-gradient-to-r from-kun-primary/10 via-kun-light/15 to-white blur-3xl opacity-60"></div>
                    <div class="absolute inset-y-0 left-1/2 -translate-x-1/2 w-[70%] bg-gradient-to-b from-white/40 to-transparent blur-[80px]"></div>
                    <div class="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <div 
                            v-for="(item, index) in (testimoni || []).filter(t => t.is_aktif)" 
                            :key="item.id"
                            class="group relative bg-white/90 backdrop-blur border border-white/40 rounded-3xl p-8 shadow-lg shadow-kun-primary/5 hover:shadow-kun-primary/30 transition duration-300 overflow-hidden"
                            data-aos="fade-up"
                            :data-aos-delay="index * 100"
                        >
                            <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <div class="absolute inset-0 bg-gradient-to-br from-kun-primary/5 via-transparent to-kun-light/10"></div>
                                <div class="absolute -top-10 right-6 w-32 h-32 bg-kun-primary/10 blur-3xl"></div>
                            </div>
                            <div class="relative z-10">
                                <div class="flex items-center justify-between mb-4">
                                    <div class="flex gap-1">
                                        <Star v-for="n in 5" :key="n" class="w-5 h-5" :class="n <= item.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'" />
                                    </div>
                                    <Quote class="w-8 h-8 text-kun-primary/40 group-hover:text-kun-primary/80 transition-colors duration-300" />
                                </div>
                                <p class="text-slate-700 text-base leading-relaxed mb-6">“{{ item.konten }}”</p>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="relative w-14 h-14">
                                            <img
                                                ...
                                                :src="item.foto_url"
                                                :alt="`Foto ${item.nama}`"
                                                class="w-14 h-14 rounded-2xl object-cover border border-white/70 shadow"
                                                loading="lazy"
                                            />
                                            <div
                                                v-else
                                                class="w-14 h-14 rounded-2xl bg-kun-primary/10 text-kun-primary font-bold flex items-center justify-center border border-white"
                                            >
                                                {{ item.nama ? item.nama.charAt(0) : '?' }}
                                            </div>
                                            <span class="absolute -right-1 -bottom-1 w-5 h-5 rounded-full bg-white text-xs font-semibold text-kun-primary border border-kun-primary/20">
                                                {{ item.rating }}
                                            </span>
                                        </div>
                                        <div>
                                            <h5 class="font-semibold text-slate-900">{{ item.nama }}</h5>
                                            <p class="text-xs text-slate-500">{{ item.jabatan }} - {{ item.perusahaan }}</p>
                                        </div>
                                    </div>
                                    <span class="text-xs uppercase tracking-widest text-slate-400 hidden lg:block">Verified</span>
                                </div>
                            </div>
                        </div>
                    </div>
