<template>
  <div class="relative h-[500px] w-full overflow-hidden">
    <div class="absolute top-0 left-0 h-full w-12 z-10 bg-gradient-to-l from-transparent to-transparent" />
    <div class="absolute top-0 right-0 h-full w-12 z-10 bg-gradient-to-r from-transparent to-transparent" />
    ...
    <div class="flex h-full items-center justify-center [perspective:1000px] [transform-style:preserve-3d]">
      ...
      >
        ...
        >
          <div
            class="pointer-events-auto relative w-full max-w-[240px] sm:max-w-[280px] lg:max-w-[320px] aspect-[9/16] overflow-hidden rounded-[15px] border-[3px] border-white transition-transform duration-300 ease-in-out will-change-transform hover:scale-105"
          >
            ...
            <img
              ...
              alt="gallery-bg"
              loading="lazy"
              class="absolute inset-0 h-full w-full object-cover blur-md scale-110 opacity-60"
            />
            <img
              :src="url"
              alt="gallery"
...
const cylinderWidth = computed(() => (isScreenSizeSm.value ? 1400 : 2600));
const faceWidth = computed(() => {
  const base = cylinderWidth.value / REFERENCE_FACE_COUNT_SIZING;
  const multiplier = isScreenSizeSm.value ? 1.45 : 1.5;
...
const trackStyle = computed(() => ({
  ...
  'transform-style': 'preserve-3d' as const
}));
