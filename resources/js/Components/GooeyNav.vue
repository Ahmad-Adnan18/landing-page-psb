...
const scrollToHash = (hash?: string) => {
  if (!hash || !hash.startsWith('#') || typeof window === 'undefined') return;
...
const resolveListElement = (target: EventTarget | null) => {
...
const handleClick = (e: Event, index: number) => {
  const liEl = resolveListElement(e.currentTarget);
  const href = props.items[index]?.href;
